...
final class RecentOrderCell: UICollectionViewCell {
...
extension RecentOrderCell {
    private func setupAttribute() {
        button.setImage(.init(systemName: "plus")?.withTintColor(.white), for: .normal)
        let title = NSMutableAttributedString.build(string: "Add", attributes: Const.buttonTitleAttributes)
        button.setAttributedTitle(title, for: .normal)
        button.contentEdgeInsets = .init(top: 8, left: 14, bottom: 10, right: 18)
...
private extension RecentOrderCell {
    enum Const {
